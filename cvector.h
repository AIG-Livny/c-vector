...
// @brief cvector_insert_data - insert elements at position pos to the vector
// @param vec - the vector
// @param pos - position in the vector where the new elements are inserted.
// @param data - value to be copied to the inserted elements.
// @param count - count of data elements
// @return void
#define cvector_insert_data(vec, pos, data, count) \
    do { \
        if( cvector_capacity(vec) < ( cvector_size(vec) + (count) ) ) { \
            cvector_reserve((vec), cvector_size(vec) + (count)); \
        } \
        memmove( (vec) + ( (pos) + (count) ), (vec) + (pos), (count) * sizeof(*(vec)) ); \
        memmove( (vec) + (pos) , (data), (count) * sizeof(*(vec)) ); \
